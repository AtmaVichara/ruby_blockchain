...
class PKI
  ...
  def self.plaintext(cyphertext, raw_public_key)
    public_key = OpenSSL::PKey::RSA.new(raw_public_key)
    public_key.public_decrypt(Base64.decode64(cyphertext))
  end

  def self.valid_signature?(message, cyphertext, public_key)
    message == plaintext(cyphertext, public_key)
  end
end
